...
func TestLocalObjectModeRoundTrip(t *testing.T) {
	...
	size, digest, err := s.PutStream(context.Background(), key, strings.NewReader(string(content)))
	...
	if _, _, err = s.PutStream(context.Background(), key, strings.NewReader(string(content))); err != nil {
